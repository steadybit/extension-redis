...
// Endpoints with an active `CLIENT PAUSE ALL` attack are skipped; the last successful
// discovery for those endpoints is returned instead so targets stay visible to the
// platform while the attack is running.
func FetchTargetsPerEndpoint(handler func(endpoint *config.RedisEndpoint) ([]discovery_kit_api.Target, error)) ([]discovery_kit_api.Target, error) {
	...
	for _, endpoint := range config.Config.Endpoints {
		...
		if IsPaused(key) {
			if cached, ok := recallTargets(key); ok {
				log.Debug().Msgf("Skipping discovery for paused endpoint %s, serving %d cached target(s)", key, len(cached))
				allTargets = append(allTargets, cached...)
			} else {
				log.Debug().Msgf("Skipping discovery for paused endpoint %s, no cached targets yet", key)
			}
			continue
		}
